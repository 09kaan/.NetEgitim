...
Gün 24
CQRS Pattern nedir?
CQRS, okuma ve veri değiştirme işlemlerini birbirinden ayırır.

Command → Veriyi değiştirir.
Query   → Veriyi okur.
Handler → Command veya Query’nin işini yapar.

Onion Architecture
→ Kod hangi katmanda bulunmalı?
→ Katmanlar hangi yönde bağımlı olmalı?

CQRS
→ Okuma ve yazma işlemleri nasıl ayrılmalı?
→ Her kullanım senaryosunu hangi Handler yürütmeli?
*/